...
if __name__ == "__main__":
    ...
    # sys.path.append('~/Projects/Parser/publish/pylat')

    from pylat.MSD import MSD
    from pylat.calcDiffusivity import calcdiffusivity
    from pylat.calcCOM import calcCOM
    from pylat.getTimeData import gettimedata
    from pylat.getMolData import getmoldata
    from pylat.COMradial import COMradialdistribution
    from pylat.getAtomCharges import getatomcharges
    from pylat.calcNEconductivity import calcNEconductivity
    from pylat.calcCond import calcCond
    from pylat.getCoordinationNumber import getcoordinationnumber
    from pylat.ionpair import ionpair
    from pylat.calcDielectricConstant import calcDielectricConstant
    from pylat.distSearch import distSearch
    from pylat.calcVisc import calcVisc
    from pylat.fitVisc import fitVisc
    ...
    File_Options.add_argument(
        "-i",
        "--input",
        help="json file if properties have already been calculated for this system",
    )
    File_Options.add_argument(
        "-f",
        "--output",
        help="File name for output file. Defult file name is output.json",
        default="output.json",
    )
    properties = parser.add_argument_group("Possible Properties")
    properties.add_argument(
        "-g", "--RDF", help="Radial Distribution Function.", action="store_true"
    )
    properties.add_argument(
        "-c",
        "--coord",
        help="Coordination Number. Will determine the first three minima of the RDF and calculates the coordination numbers integrating to those points.",
        action="store_true",
    )
    properties.add_argument(
        "-m", "--MSD", help="Mean Square Displacement.", action="store_true"
    )
    properties.add_argument(
        "-d",
        "--D",
    ...
    properties.add_argument(
        "--GKC", help="Green-Kubo ionic conductivity.", action="store_true"
    )
    ...
    properties.add_argument(
        "--DS",
        help="Search for molecules of given types at a given distance",
        action="store_true",
    )
    properties.add_argument(
        "--Visc",
        help="Calculate the viscosity of the system. Requires multiple log files in different directories. The name of the log files will be taken from the LOG option",
        action="store_true",
    )
    ...
    system.add_argument(
        "--mol",
        help="Add molecule name to list of molecules. Required",
        action="append",
        required=True,
    )
    system.add_argument(
        "--nummol",
        help="Add number of molecule type to list. Must have same number as --mol. Required",
        action="append",
        required=True,
        type=int,
    )
    system.add_argument(
        "-T",
        "--temp",
        help="Temperature of system. Required for conductivity calculations",
        type=float,
    )
    parser.add_argument(
        "-v",
        "--verbose",
        help="""Verbosity of program: \n0: No print statements \n1: Print after each major step \n2: Print after each major step and progress of the major step at each minor step. Not recommended for cluster calculations""",
        type=int,
    )

    MSDoptions = parser.add_argument_group("MSD Options")
    MSDoptions.add_argument(
        "--MSD_skip",
        help="Number of timesteps to skip at the beginning of the trajectory file before calculating MSD. Default is 0",
        default=0,
        type=int,
    )
    MSDoptions.add_argument(
        "--MSD_num_init",
        help="Number of initial timesteps to consider in MSD calculation. Default is half of frames being used in MSD calculation",
        default=None,
    )

    Difoptions = parser.add_argument_group("Diffusivity Options")
    Difoptions.add_argument(
        "--D_Tolerance",
        help="Tolerance for determining linear region in Log-Log plot. Default is 0.075",
        type=float,
        default=0.075,
    )

    RDFoptions = parser.add_argument_group("RDF Options")
    RDFoptions.add_argument(
        "--RDF_Timesteps",
        help="Number of timesteps to use in the RDF calculation. \nWill use the last n timesteps. \nDefault is to use all timesteps",
        type=int,
    )
    RDFoptions.add_argument(
        "--RDF_maxr",
        help="Maximum r for RDF calculation. Default is half the shortest box length.",
        default=None,
    )
    RDFoptions.add_argument(
        "--RDF_binsize",
        help="Bin size for RDF calculation. Default is 0.1",
        default=0.1,
        type=float,
    )

    GKCoptions = parser.add_argument_group("GK Conductivity options")
    GKCoptions.add_argument(
        "--GKC_skip",
        help="Number of timesteps to skip at the beginning of the trajectory file before calculating GK Conductivity. Default is 0",
        default=0,
        type=int,
    )
    GKCoptions.add_argument(
        "--GKC_Tolerance",
        help="Tolerance for finding the converged region for GK conductivity calculation. Default is 0.001",
        default=0.001,
        type=float,
    )
    GKCoptions.add_argument(
        "--GKC_J_Output",
        help="Option to output the charge flux correlation function. If included, a file J.dat will be created with the values of the correlation function.",
        action="store_true",
    )

    IPLoptions = parser.add_argument_group("Ion Pair Lifetime options")
    IPLoptions.add_argument(
        "--IPL_skip",
        help="Number of timesteps to skip at the beginning of the trajectory file before calculating Ion Pair Lifetime. Default is 0",
        default=0,
        type=int,
    )

    DECoptions = parser.add_argument_group("Dielectric Constant options")
    DECoptions.add_argument(
        "--DEC_start",
        help="Starting frame for dielectric constant calculations. Default is 1",
        default=1,
        type=int,
    )

    DSoptions = parser.add_argument_group("Distance Search Options")
    DSoptions.add_argument(
        "--DS_MOL1",
        help="Molecule name for the first molecule type. Required for Distance Search",
    )
    DSoptions.add_argument(
        "--DS_MOL2",
        help="Molecule name for the second molecule type. Required for Distance Search",
    )
    DSoptions.add_argument(
        "--DS_Dist",
        help="Distance between molecules. Required for Distance Search",
        type=float,
    )
    DSoptions.add_argument(
        "--DS_Dist_Tol",
        help="Tolerance for distance. Required for Distance Search",
        type=float,
    )
    DSoptions.add_argument(
        "--DS_First_Frame",
        help="First frame for distance search calculation. Default is 1",
        default=1,
        type=int,
    )
    DSoptions.add_argument(
        "--DS_Num_Samples",
        help="Number of samples to find. Default is 1",
        default=1,
        type=int,
    )

    Viscoptions = parser.add_argument_group("Viscosity options")
    Viscoptions.add_argument(
        "--Visc_Dir",
        help='Basename for the viscosity directories. For example, if basename is "visc" the directories should be visc1, visc2, ... \n If no base name is given, directories should be 1, 2, 3, ...',
        default=None,
    )
    Viscoptions.add_argument(
        "--Visc_Num",
        help="Number of Trajectories to use in the viscosity calculation. Each trajectory should have a seperate trajectory",
        type=int,
    )
    Viscoptions.add_argument(
        "--Visc_Skip",
        help="Number of timesteps to skip in the viscosity calculation. Default of 0 should only be used if the simulations were equilibrated before the start of the production run",
        default=0,
        type=int,
    )
    Viscoptions.add_argument(
        "--Visc_Num_Boots",
        help="Number of times to sample trajectories for bootstrapping. Default is 10",
        default=10,
        type=int,
    )
    Viscoptions.add_argument(
        "--Visc_Samples",
        help="Number of samples for each bootstrapping iteration. Default is 30",
        default=30,
        type=int,
    )
    Viscoptions.add_argument(
        "--Visc_Plot",
        help="Include plotting for viscosity fitting. Matplotlib required for fitting. Will output to x11",
        action="store_true",
    )
    Viscoptions.add_argument(
        "--Visc_Guess",
        help='Initial guess for viscosity fitting parameters. Must add all four parameters with one call. Order of parameters is A, alpha, tau1, tau2. Default is the equivalent of calling "--Visc_Guess 1e-3 1.5e-1 1e2 1e3".',
        type=float,
        nargs=4,
    )
    ...
    if arg.RDF or arg.MSD or arg.IPL:
        ...
        (comx, comy, comz, Lx, Ly, Lz, Lx2, Ly2, Lz2) = c.calcCOM(
            trjfilename, datfilename, ver
        )
        if arg.verbose == 1:
            print("COM calculation complete")

    if arg.RDF:
        if arg.RDF_Timesteps is None:
    ...
    if arg.DEC:
        output = dec.calcDEC(
            atomcharges, trjfilename, arg.temp, output, V, arg.verbose, arg.DEC_start
        )

    if arg.GKC:
        if arg.verbose >= 1:
            print("beginning GK conductivity calculation")
        output = cc.calcConductivity(
            molcharges,
            trjfilename,
            logfilename,
            datfilename,
    ...
    if arg.NEC:
        ...
        output = ne.calcNEconductivity(
            output, molcharge, Lx, Ly, Lz, nummoltype, moltypel, arg.temp
        )
        if arg.verbose >= 1:
