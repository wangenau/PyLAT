...
def aver(x, y):
    xstart = float(input("Select Starting x value (-1 for last point): "))
    if xstart == -1:
        print(y[-1])
    else:
        xend = float(input("Select ending x value: "))
        xstartind = None
        xendind = None
        for i in range(0, len(x)):
            if x[i] > xstart and xstartind is None:
                xstartind = i
            if x[i] > xend and xendind is None:
                xendind = i
...
def dic(data):
    ...
    if isinstance(data[keylist[select]], list):
        ...
        if select2 == -1:
            ...
            y = data[keylist[select]]
        plt.show()
        select3 = input("Would you like to calculate an average or get last point? ")
        if select3.lower() == "yes" or select3.lower() == "y":
            aver(x, y)
        select4 = input("Would you like to save a data file? ")
