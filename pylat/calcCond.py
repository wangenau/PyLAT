    ...
    def calcConductivity(
        ...
        for i in range(len(trjfilename)):
            ...
            while count < num_timesteps:
                ...
                if count == 0:
                    (comvx, comvy, comvz, nummol, molmass, jx, jy, jz) = self.COMprep(
                        mol, atype, atommass, n, num_timesteps, moltypel
                    )
                (comvx, comvy, comvz) = self.calcCOMv(
                    comvx,
                    comvy,
                    comvz,
                    vx,
                ...
                if ver == 2:
                    sys.stdout.write(f"\rCOM velocity calculation {count * 100 / num_timesteps:.2f}% complete")
        if ver == 2:
            sys.stdout.write("\n")
        if ver >= 2:
    ...
    def calcJ(self, jx, jy, jz, ver):
        ...
        for i in range(len(jx)):
            for m in range(len(jx)):
                ...
                counter += 1
                if ver == 2:
                    sys.stdout.write(f"\rGK conductivity correlation {100 * float(counter) / len(jx) ** 2}% complete")
        if ver == 2:
            sys.stdout.write("\n")
        return J
