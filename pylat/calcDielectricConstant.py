...
class calcDielectricConstant:
    def calcDEC(self, atomcharges, trjfilename, T, output, V, ver, start):
        ...
        output["Dielectric Constant"]["explanation"] = (
            "returns a list of the cumulative dielectric constant for the number of timesteps used in the averages to show convergence."
        )
    ...
    def readdata(
        self, trjfile, n, line, x, y, z, mol, xcol, ycol, zcol, molcol, idcol, i
    ):
    ...
    def calcaverage(self, Mx, My, Mz, start):
        ...
        AveM2 = (
            np.cumsum(Mx[start:] ** 2 + My[start:] ** 2 + Mz[start:] ** 2) / normarray
        )
        AveM = (
            np.cumsum(Mx[start:]) ** 2
            + np.cumsum(My[start:]) ** 2
            + np.cumsum(Mz[start:]) ** 2
        ) / normarray**2
        return (AveM2, AveM)
